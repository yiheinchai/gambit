  ...
  return (
    <div style={{ width: 1280, height: 920, background: "var(--bg)", fontFamily: "var(--sans)", padding: "24px 40px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 20 }}>
        <div>
          <div style={{ fontSize: 11, fontWeight: 800, color: "var(--ink-3)", textTransform: "uppercase", letterSpacing: 0.5 }}>Tuesday · Apr 30</div>
          <h1 style={{ fontSize: 42, fontWeight: 900, letterSpacing: -1.2, margin: "4px 0 0", lineHeight: 1.05 }}>Daily quest</h1>
        </div>
        <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
          <div style={{ background: "var(--orange)", color: "white", padding: "10px 18px", borderRadius: 14, boxShadow: "0 4px 0 var(--orange-dark)", display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ fontSize: 22 }}>🔥</span>
            <div><div style={{ fontSize: 18, fontWeight: 900, lineHeight: 1 }}>11 day</div><div style={{ fontSize: 10, fontWeight: 800, opacity: 0.9, letterSpacing: 0.4, textTransform: "uppercase" }}>streak</div></div>
          </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 22 }}>
        <div style={{ background: "white", border: "3px solid var(--ink)", borderRadius: 22, padding: 28, boxShadow: "0 6px 0 var(--ink)", position: "relative", height: 720 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 18 }}>
            <div>
              ...
              <div style={{ fontSize: 22, fontWeight: 900, marginTop: 2, letterSpacing: -0.5 }}>Trapped Pieces · Lesson 3</div>
            </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ background: "white", border: "3px solid var(--ink)", borderRadius: 18, padding: 18, boxShadow: "0 5px 0 var(--ink)" }}>
            ...
            <Quest icon="🎯" title="Solve 10 puzzles" sub="from your weak concepts" done={7} total={10} reward="40 💎" />
            <Quest icon="🦾" title="Find a brilliant move" sub="engine top-1 in any drill" done={1} total={1} reward="20 💎" complete />
            <Quest icon="📖" title="Read 1 concept page" sub="Concept Library" done={0} total={1} reward="15 💎" />
            <Quest icon="⚔️" title="Win 1 ranked drill battle" sub="vs another user" done={0} total={1} reward="50 💎" />
          </div>
          ...
          <div style={{ background: "var(--ink)", borderRadius: 18, padding: 16, color: "white" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                ...
                <div style={{ fontSize: 14, fontWeight: 900, marginTop: 2 }}>Protect your 11-day streak</div>
              </div>
...
function Path() {
  // Duolingo-style winding path with 5 nodes
  const nodes = [
    { y: 50, x: 0.45, status: "done", icon: "✓", label: "Warm-up · 5 puzzles" },
    { y: 160, x: 0.65, status: "done", icon: "✓", label: "Pattern recognition" },
    { y: 270, x: 0.45, status: "current", icon: "⚔", label: "Replay your blunders", desc: "3 of your trapped-knight positions. Find Bxf3 ideas." },
...
function PathNode({ y, x, status, icon, label, desc, boss }) {
  const colors = {
    ...
    <div style={{ position: "absolute", top: y - size/2, left: `calc(${x*100}% - ${size/2}px)`, width: size, height: size }}>
      ...
      {label && (
        <div style={{ position: "absolute", left: size + 18, top: size/2 - 18, background: "white", border: "2.5px solid var(--ink)", borderRadius: 12, padding: "8px 12px", boxShadow: "0 4px 0 var(--ink)", whiteSpace: "nowrap", maxWidth: 280 }}>
          ...
          {status === "current" && <button style={{ marginTop: 6, background: "var(--orange)", color: "white", border: "none", padding: "6px 12px", borderRadius: 8, fontFamily: "var(--sans)", fontWeight: 900, fontSize: 11, letterSpacing: 0.5, textTransform: "uppercase", boxShadow: "0 3px 0 var(--orange-dark)", cursor: "pointer" }}>Start →</button>}
        </div>
      )}
    </div>
  );
...
function Quest({ icon, title, sub, done, total, reward, complete }) {
  return (
