...
import { getGamesByUsername } from "../lib/db";
...
export default function Leagues() {
  const { username, games: storeGames } = useApp();
  const navigate = useNavigate();
  const [playerElo, setPlayerElo] = useState(1842);
  ...
  // Compute real elo from games
  useEffect(() => {
    async function loadElo() {
      if (!username) return;
      // Try store games first, then fall back to DB
      let games = storeGames;
      if (games.length === 0) {
        games = await getGamesByUsername(username);
      }
      if (games.length > 0) {
        // Use the most recent game's elo
        const sorted = [...games].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
        setPlayerElo(sorted[0].playerElo);
      }
    }
    loadElo();
  }, [username, storeGames]);

  const realName = username || "magnus_fan_42";

  const players: Player[] = [
    { rank: 1, name: "knight_rider_88", elo: 1947, xp: 4820, change: "+312", you: false, friend: false, badge: "👑" },
    { rank: 2, name: "endgame_eli", elo: 1902, xp: 4410, change: "+241", friend: true },
    { rank: 3, name: realName, elo: playerElo, xp: 4180, change: "+267", you: true },
  ...
  const badges = [
    ...
    { g: "🏆", c: "var(--yellow-dark)", on: false, n: "Champion" },
    { g: "🦉", c: "var(--ink)", on: false, n: "Owl" },
    { g: "♛", c: "#5B21B6", on: false, n: "Master" },
    { g: "💎", c: "#06B6D4", on: false, n: "Diamond" },
  ];
...
function ChipL({ children, active }: { children: React.ReactNode; active?: boolean }) {
...
function LeagueRow({ rank, name, elo, xp, change, you, friend, badge, promo }: { rank: number; name: string; elo: number; xp: number; change: string; you?: boolean; friend?: boolean; badge?: string; promo?: boolean }) {
...
function Avatar({ n, c, dark }: { n: string; c: string; dark?: boolean }) {
...
function Activity({ who, what, t, g }: { who: string; what: string; t: string; g: string }) {
  return (
