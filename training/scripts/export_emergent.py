"""
Export the combined model (board → SAE features) to ONNX for browser inference.

Usage:
    python export_emergent.py --move-checkpoint ../models/move_predictor_best.pt --sae ../models/sae.pt --output ../../public/models/concept_classifier.onnx
"""

import argparse
import json
from pathlib import Path

import numpy as np
import onnx
import torch
import torch.nn as nn

from move_predictor import MovePredictor
from sparse_autoencoder import SparseAutoencoder


class EmergentConceptModel(nn.Module):
    """Combined model: board tensor → sparse SAE features."""

    def __init__(self, backbone: MovePredictor, sae: SparseAutoencoder):
        super().__init__()
        self.input_proj = backbone.input_proj
        self.blocks = backbone.blocks
        self.pool = backbone.pool
        self.flatten = backbone.flatten
        self.sae_encoder = sae.encoder
        self.sae_bias = sae.bias
        self.k = sae.k
        self.dict_size = sae.dict_size

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = self.input_proj(x)
        x = self.blocks(x)
        x = self.pool(x)
        x = self.flatten(x)

        # SAE encode
        x = x - self.sae_bias
        z = torch.relu(self.sae_encoder(x))

        # TopK sparsity
        topk_vals, topk_idx = z.topk(self.k, dim=-1)
        sparse_z = torch.zeros_like(z)
        sparse_z.scatter_(-1, topk_idx, topk_vals)
        return sparse_z


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--move-checkpoint", required=True)
    parser.add_argument("--sae", required=True)
    parser.add_argument("--concepts-json", default="../models/emergent_concepts.json")
    parser.add_argument("--output", required=True)
    parser.add_argument("--channels", type=int, default=128)
    parser.add_argument("--blocks", type=int, default=6)
    args = parser.parse_args()

    # Load SAE
    sae_data = torch.load(args.sae, map_location="cpu", weights_only=True)
    sae = SparseAutoencoder(sae_data["input_dim"], sae_data["dict_size"], sae_data["k"])
    sae.load_state_dict(sae_data["state_dict"])

    # Load move predictor (need vocab size — infer from checkpoint)
    state = torch.load(args.move_checkpoint, map_location="cpu", weights_only=True)
    vocab_size = state["head.weight"].shape[0]
    backbone = MovePredictor(vocab_size, args.channels, args.blocks)
    backbone.load_state_dict(state)

    # Combine
    model = EmergentConceptModel(backbone, sae)
    model.eval()

    # Test
    dummy = torch.randn(1, 15, 8, 8)
    out = model(dummy)
    active = (out > 0).sum().item()
    print(f"Output shape: {out.shape}, active features: {active}, k={sae_data['k']}")

    # Export
    output_path = Path(args.output)
    output_path.parent.mkdir(parents=True, exist_ok=True)

    torch.onnx.export(
        model, dummy, str(output_path),
        export_params=True,
        opset_version=18,
        input_names=["board"],
        output_names=["concepts"],
        dynamic_axes={"board": {0: "batch"}, "concepts": {0: "batch"}},
    )

    # Embed weights into single file
    m = onnx.load(str(output_path))
    onnx.save_model(m, str(output_path), save_as_external_data=False)
    for f in output_path.parent.glob("*.onnx.data"):
        f.unlink()

    size_mb = output_path.stat().st_size / 1024 / 1024
    print(f"Exported: {output_path} ({size_mb:.1f} MB)")

    # Validate with onnxruntime
    import onnxruntime as ort
    sess = ort.InferenceSession(str(output_path))
    result = sess.run(None, {"board": dummy.numpy()})[0]
    print(f"ONNX validation: shape={result.shape}, active={int((result > 0).sum())}")

    # Write concepts manifest
    features_info = []
    if Path(args.concepts_json).exists():
        with open(args.concepts_json) as f:
            data = json.load(f)
            features_info = data.get("features", [])

    concept_names = []
    for i in range(sae_data["dict_size"]):
        feat = next((f for f in features_info if f["id"] == i), None)
        if feat:
            concept_names.append(feat["name"])
        else:
            concept_names.append(f"feature_{i}")

    manifest = {
        "type": "emergent",
        "concept_names": concept_names,
        "concept_dim": sae_data["dict_size"],
        "k": sae_data["k"],
        "params": sum(p.numel() for p in model.parameters()),
    }
    manifest_path = output_path.parent / "concepts.json"
    with open(manifest_path, "w") as f:
        json.dump(manifest, f, indent=2)
    print(f"Manifest: {manifest_path}")


if __name__ == "__main__":
    main()
